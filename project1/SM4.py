import struct
import time
# SM4算法常量定义
SM4_BLOCK_SIZE = 16
SM4_KEY_SIZE = 16
SM4_ROUNDS = 32

# S盒
SBOX = [
    
]

# 系统参数FK
FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc]

# 固定参数CK
CK = [
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
]

def rotate_left(x, n):
    #循环左移
    return ((x << n) | (x >> (32 - n))) & 0xFFFFFFFF

def tau(a):
    #非线性变换τ(.)
    a0 = (a >> 24) & 0xFF
    a1 = (a >> 16) & 0xFF
    a2 = (a >> 8) & 0xFF
    a3 = a & 0xFF
    b0 = SBOX[a0]
    b1 = SBOX[a1]
    b2 = SBOX[a2]
    b3 = SBOX[a3]
    return (b0 << 24) | (b1 << 16) | (b2 << 8) | b3

def l(b):
    #线性变换L
    return b ^ rotate_left(b, 2) ^ rotate_left(b, 10) ^ rotate_left(b, 18) ^ rotate_left(b, 24)

def l_prime(b):
    #线性变换L
    return b ^ rotate_left(b, 13) ^ rotate_left(b, 23)

def t(x):
   #合成变换T
    return l(tau(x))

def t_prime(x):
    #合成变换T
    return l_prime(tau(x))

def f(x0, x1, x2, x3, rk):
    #轮函数F
    return x0 ^ t(x1 ^ x2 ^ x3 ^ rk)

def key_expansion(key):
    # 将密钥转换为4个字
    mk = [0] * 4
    for i in range(4):
        mk[i] = (key[4*i] << 24) | (key[4*i+1] << 16) | (key[4*i+2] << 8) | key[4*i+3]    
    k = [0] * 36
    for i in range(4):
        k[i] = mk[i] ^ FK[i]    
    rk = [0] * SM4_ROUNDS
    for i in range(32):
        k[i+4] = k[i] ^ t_prime(k[i+1] ^ k[i+2] ^ k[i+3] ^ CK[i])
        rk[i] = k[i+4]    
    return rk

def sm4_crypt(input_data, rk, crypt_type='encrypt'):
    # 将输入数据转换为4个字
    x = [0] * 36
    for i in range(4):
        x[i] = (input_data[4*i] << 24) | (input_data[4*i+1] << 16) | (input_data[4*i+2] << 8) | input_data[4*i+3]
    # 32轮迭代运算
    for i in range(32):
        if crypt_type == 'encrypt':
            rk_i = rk[i]
        else:  # decrypt
            rk_i = rk[31 - i]
        x[i+4] = f(x[i], x[i+1], x[i+2], x[i+3], rk_i)
    # 反序变换
    output_data = bytearray(16)
    y = [x[35], x[34], x[33], x[32]]
    for i in range(4):
        output_data[4*i] = (y[i] >> 24) & 0xFF
        output_data[4*i+1] = (y[i] >> 16) & 0xFF
        output_data[4*i+2] = (y[i] >> 8) & 0xFF
        output_data[4*i+3] = y[i] & 0xFF   
    return output_data

def pad(data):
    #PKCS#7填充
    pad_len = SM4_BLOCK_SIZE - (len(data) % SM4_BLOCK_SIZE)
    return data + bytes([pad_len] * pad_len)

def unpad(data):
    #PKCS#7去填充
    pad_len = data[-1]
    return data[:-pad_len]

def sm4_encrypt(plaintext, key):
    #SM4加密
    if len(key) != SM4_KEY_SIZE:
        raise ValueError("Key must be 16 bytes long")
    # 密钥扩展
    rk = key_expansion(key)
    # 填充明文
    padded_plaintext = pad(plaintext)
    # 分块加密
    ciphertext = bytearray()
    for i in range(0, len(padded_plaintext), SM4_BLOCK_SIZE):
        block = padded_plaintext[i:i+SM4_BLOCK_SIZE]
        encrypted_block = sm4_crypt(block, rk, 'encrypt')
        ciphertext.extend(encrypted_block)
    return bytes(ciphertext)

def sm4_decrypt(ciphertext, key):
    #SM4解密
    if len(key) != SM4_KEY_SIZE:
        raise ValueError("Key must be 16 bytes long")
    if len(ciphertext) % SM4_BLOCK_SIZE != 0:
        raise ValueError("Ciphertext length must be a multiple of 16 bytes") 
    rk = key_expansion(key)  
    # 分块解密
    plaintext = bytearray()
    for i in range(0, len(ciphertext), SM4_BLOCK_SIZE):
        block = ciphertext[i:i+SM4_BLOCK_SIZE]
        decrypted_block = sm4_crypt(block, rk, 'decrypt')
        plaintext.extend(decrypted_block)   
    # 去除填充
    return unpad(plaintext)

# 测试
if __name__ == "__main__":
    # 测试密钥和明文
    key = bytes.fromhex("0123456789abcdeffedcba9876543210")
    plaintext = b"0123456789abcdeffedcba9876543210"
    print(f"Original plaintext: {plaintext.decode()}")
    # 加密
    ciphertext = sm4_encrypt(plaintext, key)
    print(f"Ciphertext (hex): {ciphertext.hex()}")
    # 解密
    decrypted = sm4_decrypt(ciphertext, key)
    print(f"Decrypted plaintext: {decrypted.decode()}")
    # 验证
    assert decrypted == plaintext, "Decryption failed!"
    print("Test passed! Plaintext matches after encryption and decryption.")